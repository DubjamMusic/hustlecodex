...
export class CycleOrchestrator {
  ...
  async executeDirective(
    ...
    console.log('[Orchestrator] Executing both teams in parallel...');
    const [alphaOutputs, omegaOutputs] = await Promise.all([
    ...
    const [alphaValidation, omegaValidation] = await Promise.all([
